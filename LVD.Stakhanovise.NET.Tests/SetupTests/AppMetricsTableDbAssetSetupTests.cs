...
namespace LVD.Stakhanovise.NET.Tests.SetupTests
{
	...
	public class AppMetricsTableDbAssetSetupTests : BaseSetupDbTests
	{
		...
		{
			AppMetricsTableDbAssetSetup setup = new AppMetricsTableDbAssetSetup();

			await setup.SetupDbAssetAsync( GetSetupTestDbConnectionOptions(), mapping );
		...
		private QueuedTaskMapping GenerateNonDefaultMapping ()
		{
			...
			mapping.MetricsTableName = faker.Lorem.Letter( 5 ) + "_" + mapping.MetricsTableName;
			return mapping;
		}
	}
}
