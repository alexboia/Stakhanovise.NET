...
export type DocumentKind = "dbdef" | "dbmap";
...
export function parseDocument(kind: DocumentKind, text: string): ParsedDiagnostic[] {
  return kind === "dbmap" ? parseDbMapText(text) : parseDbDefText(text);
}

export function parseDbMapText(text: string): ParsedDiagnostic[] {
...
export function parseDbDefText(text: string): ParsedDiagnostic[] {
  const diagnostics: ParsedDiagnostic[] = [];
  const lines = text.split(/\r?\n/);
  let inBody = false;
...
function getDirective(trimmed: string): string | undefined {
...
function validateProps(line: string, lineIndex: number, diagnostics: ParsedDiagnostic[]): void {
  ...
  const entries = rest.split(";").map((entry: string) => entry.trim()).filter(Boolean);
...
function validateDetailWithRequiredProps(
  line: string,
  lineIndex: number,
  directive: string,
  requiredKeys: string[],
...
function validateConstraint(line: string, lineIndex: number, diagnostics: ParsedDiagnostic[]): void {
  const trimmed = line.trim();
...
function validateIndex(line: string, lineIndex: number, diagnostics: ParsedDiagnostic[]): void {
  const trimmed = line.trim();
...
function validateReturn(line: string, lineIndex: number, diagnostics: ParsedDiagnostic[]): void {
  const trimmed = line.trim();
